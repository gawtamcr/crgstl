...
def collect_expert_transitions(env: STLGymWrapper, controller: SafeFunnelController, 
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            ...
            # Get current phase info from wrapper (derived from conductor)
            phase, safety, t_left = env.get_wrapper_attr('get_current_phase_info')()
            
            # Get expert action
            action = controller.get_action(phase, safety, raw_obs, t_left)
            
            # Force gripper to stay closed during the 'move' phase
            if phase == "move":
                action[3] = -1.0
